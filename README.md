...
- **Menu Management**: The menu is edited directly in the Pages tab via "Menu Section" blocks (accordion sections → groups → dishes with flexible price lines). Run `npm run migrate:menu` once to convert the data from the old menu collections into a `speisekarte` page; the old collections are hidden from the admin and can be deleted after verifying the result
